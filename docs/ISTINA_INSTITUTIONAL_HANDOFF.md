...
6. A completed, service-owner/operations-approved copy of
   `config/istina_online_load_plan.template.json`. It must bind the exact
   dataset, code revision, endpoint, man ID, load parameters, change ticket,
   and timezone-aware execution window before the first request.
7. Five retained JSON deployment attachments: shadow telemetry, the immutable
   online-load plan, online-load output, a completed
   `config/istina_drift_monitor_verification.template.json`, and an audit
   retention artifact generated by `evaluation/istina_audit_retention.py` from
   the private retained chain and shadow telemetry for every worker. The audit
   template documents the output schema; it must not be completed by hand.
8. A completed copy of
   `config/istina_deployment_evidence.template.json` that binds those four
   operational artifacts plus the load plan, the exact data hash, and the exact
   40-hex Git revision.
...
# Complete the intentionally invalid load-plan template and obtain approval
# before the window. These helpers print only hashes; keep the plan private.
python -c "from evaluation.istina_online_load_plan import sha256_text; print(sha256_text('<approved-service-url>'))"
python -c "from evaluation.istina_online_load_plan import man_id_sha256; print(man_id_sha256(<approved-man-id>))"

# Preflight may run before a future approved window. Every argument is compared
# exactly with the plan. Exit status is nonzero if any binding is invalid.
python evaluation/istina_online_load_plan.py --plan <private-online-load-plan.json> --expected-dataset <private-istina-export.json> --expected-code-revision $revision --service-url <approved-service-url> --man-id <approved-man-id> --requests 1000 --concurrency 4 --max-rps 2 --service-timeout 30 --approved-change-reference <change-ticket> --output <private-online-load-plan-preflight.json>

# Run only inside the approved operations window. The runner repeats preflight
# with an active-window requirement before contacting the read-only endpoint.
python experiments/istina_online_read_load.py --dataset <private-istina-export.json> --requests 1000 --concurrency 4 --max-rps 2 --man-id <approved-man-id> --service-url <approved-service-url> --service-timeout 30 --code-revision $revision --approved-change-reference <change-ticket> --approval-scope institutional_load_window --load-plan <private-online-load-plan.json> --acknowledge-read-only-load --output <private-online-load.json>
...
python evaluation/istina_audit_retention.py --audit-chain <private-retained-audit-worker1.jsonl> [<private-retained-audit-worker2.jsonl>] --shadow-telemetry <private-live-shadow-worker1.json> [<private-live-shadow-worker2.json>] --dataset-sha256 <64-hex-private-dataset-sha256> --code-revision $revision --retention-days 90 --storage-reference <approved-storage-reference> --retention-policy-reference <approved-retention-policy> --output <private-audit-verification.json>

python evaluation/istina_deployment_evidence.py --manifest <private-deployment-manifest.json> --attachment <private-live-shadow.json> <private-online-load.json> <private-online-load-plan.json> <private-drift-verification.json> <private-audit-verification.json> --expected-dataset <private-istina-export.json> --expected-code-revision $revision --output <private-deployment-validation.json>
...
recomputes 55 checks from the manifest, parses all five JSON attachments, and
...
and exact file hashes. It also recomputes all 21 immutable load-plan checks at
the recorded execution-start timestamp and requires the load result to bind
the exact plan filename and SHA-256. For audit evidence it additionally recomputes the
deterministic multi-chain manifest root, checks per-worker chain/telemetry
cardinality and record totals, requires retained fsync telemetry binding, and
rejects record-level content. It also requires two distinct approval
references.
