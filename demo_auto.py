# -*- coding: utf-8 -*-
"""
自动运行演示版本（无交互） / Автоматическая демонстрация (без взаимодействия)
...
def create_representative_test_cases() -> List[AuthorRecord]:
    ...
    test_records = [
        ...
        # Case 1: 明显相同的作者 - 第二条记录（微小差异，高重合度）/ Явно тот же автор - вторая запись
        AuthorRecord(
            record_id="R002",
...
def demonstrate_incremental_disambiguation():
    """
    演示增量消歧的完整流程（自动模式）/ Демонстрация полного процесса инкрементального устранения неоднозначности (автоматический режим)
    """
    print(">>> 启动增量作者消歧系统完整原型演示 (自动模式)")
    print(">>> Запуск полной демонстрации прототипа системы инкрементального устранения неоднозначности авторов (автоматический режим)")
    ...
    for i, record in enumerate(test_records, 1):
        ...
        # 短暂暂停以便观察 / Короткая пауза для наблюдения
        time.sleep(0.5)
